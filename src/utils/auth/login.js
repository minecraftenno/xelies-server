...
module.exports = (app) => {
    app.post('/login', (req, res) => {
        ...
        if(typeof username == "undefined" && typeof email == "undefined") {
            return res.status(503).json(ApiError.badrequest)
        }
                ...
                if(doc) {
                    bcrypt.compare(password, doc.password, (err, result) => {
                        if(result == true) {
                            ...
                            return res.status(403).json(ApiError(403, "Invalid username or password."))
                        }
                    });
                } else {
                    return res.status(403).json(ApiError(403, "Invalid username or password."))
                ...
                if(doc) {
                    bcrypt.compare(password, doc.password, (err, result) => {
                        if(result == true) {
                            ...
                            return res.status(403).json(ApiError(403, "Invalid email or password."))
                        }
                    });
                } else {
                    return res.status(403).json(ApiError(403, "Invalid email or password."))
